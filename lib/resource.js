    ...
    if(t == "json") {
        ...
        subtypes.forEach(function(subType) {
            ...
            global[subType.__resource_name] = function(block) {
                ...
                if(subType.Builder === undefined) {
                    throw new Error("Builder not set properly on Resource");
                }
                logger.debug("Running builder for", subType.__resource_name);
                var builder = new subType.Builder(s);
...
Resource.registerSubtype = function(subtype, name) {
    // now, inherit the subtype's builder from ours.
    if(subtype.hasOwnProperty("Builder")) {
        logger.debug(name, "has a builder.  Setting it up.");
        inherits(subtype.Builder, this.Builder);
    }
    inherits(subtype, this);
    if(subtype.super_ === undefined) {
        throw new Error("WDSDSDSF");
    }
    ...
    logger.debug("Registered", subtype.__resource_name, "*into*", this.__resource_name, "");
    // if(this.subtypes === undefined) {
...
/**
 * Called on startup by diatropikon.js.  Walks all of the JS files in
 * lib/resources and requires them.  This is done so that the
 * subclasses can register themselves, thus making them available in
 * the DSL.
 */
Resource.walkAndLoadAll = function() {
    logger.debug("Walking resource class directory, loading everything.");
    var dir = path.resolve(path.join(__dirname, "resources"));
    ...
    return new Promise(function(resolve, reject) {
        ...
        walker.on("file", function(root, fileStats, next) {
            if(path.extname(fileStats.name) === ".js") {
                logger.debug("Dyamically loading resource definition", fileStats.name);
        ...
        walker.on("errors", function(errors) {
            reject(errors);
            // next();
        });
    }); 
